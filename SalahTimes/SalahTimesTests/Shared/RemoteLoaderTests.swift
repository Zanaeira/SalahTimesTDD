...
class RemoteLoaderTestsTests: XCTestCase {
		...
		func test_load_doesNotDeliverResultsAfterSUTInstanceHasBeenDeallocated() {
				...
				var sut: RemoteLoader? = RemoteLoader(client: httpClient)
				let (_, data) = salahTimesModelAndDataFor5thAug2021LondonUK()
		...
		private func makeSUT(file: StaticString = #file, line: UInt = #line) -> (RemoteLoader: RemoteLoader, httpClient: HTTPClientSpy, endpoint: Endpoint) {
				let httpClient = HTTPClientSpy()
				let endpointSpy = EndpointSpy.make()
				let sut = RemoteLoader(client: httpClient)
		...
		private func expect(_ sut: RemoteLoader, toCompleteWith result: TimesLoader.Result, using endpoint: Endpoint, when action: () -> Void, file: StaticString = #file, line: UInt = #line) {
				var capturedResults = [TimesLoader.Result]()
